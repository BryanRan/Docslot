    ...
    <div class="border-b border-gray-200 my-6">
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="rendezvousTabs" role="tablist">
            <li role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg text-green-600 border-green-600 hover:text-gray-600 hover:border-gray-300"
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex  items-end gap-6">
                                ...
                                <?php if ($status !== 'annulé' && $status !== 'refusé'): ?>
                                    <button
                                        class="mt-3 px-3 py-1 bg-red-600 text-white rounded-md hover:bg-red-700 transition w-28 cancel-btn"
                                        data-id="<?= $rdv['id'] ?>">
                                        Annuler
                                    </button>
...
<!-- Modal Annulation -->
<div id="cancelModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center transition-opacity duration-300">
    <div class="bg-white rounded-lg shadow-lg w-96 p-6 transform transition-transform duration-300 scale-90">
        ...
        <form id="cancelForm" method="POST">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" id="cancelClose" class="px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300">Annuler</button>
                <button type="submit" id="cancelConfirm" class="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700">Confirmer</button>
            </div>
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        venir.addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('cancel-btn')) {
                currentRdvId = e.target.dataset.id; // ✅ on récupère l'ID du bouton cliqué
                if (!currentRdvId) {
                    alert("Erreur : aucun ID trouvé sur ce bouton.");
                    return;
                }

                // ✅ on met à jour l'action du formulaire avec l'ID
                cancelForm.action = `${window.location.origin}/patient/appointment/annuler/${currentRdvId}`;
